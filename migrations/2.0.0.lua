...
local function deserialise_old_spidertron_data(spidertron, serialised_data)
...
local function migrate_serialised_data(serialised_data, surface)
    ...
    assert(spidertron, "Failed migration spidertron data: "..surface.name)

    serialised_data.driver = nil
    serialised_data.passenger = nil
    serialised_data.walking_state = nil
    serialised_data.players_with_gui_open = nil
    deserialise_old_spidertron_data(spidertron, serialised_data)
    local new_serialised_data = spidertron_lib.serialise_spidertron(spidertron)
    spidertron.destroy()

    return new_serialised_data
end


for _, dock_data in pairs(storage.docks) do
    -- Update to 2.0's way of doing render objects
    local render_objects = { }
    for _, object_id in pairs(dock_data.docked_sprites) do
        table.insert(render_objects, rendering.get_object_by_id(object_id --[[@as uint64]]))
    end
    dock_data.docked_sprites = render_objects

    -- To update serialized spiders we do something similar to Spidertron Enhancements.
    -- We will create a new spider, deserialize the data into it, then serialize that into
    -- the new format, and delete the spider.
    if dock_data.serialised_spider then
        local surface = dock_data.dock_entity and dock_data.dock_entity.valid and dock_data.dock_entity.surface or nil
        dock_data.serialised_spider = migrate_serialised_data(dock_data.serialised_spider, surface or game.get_surface("nauvis"))
    end
end